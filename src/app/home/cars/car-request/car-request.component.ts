...
export class CarRequestComponent implements OnInit {
  ...
  constructor(
    ...
    this.courrentdate = moment( new Date(currentYear , currentMounth, currentDay));
  }
  ngOnInit() {
    this.spn.show();
  ...
  changePrice() {
    ...
    this.openSnackBar("Invalid date ",'eror')
  ...
  getClient(clientKey) {
    ...
    (err) => {
      ...
      if(err.code === 'auth/user-not-found'){
        this.openSnackBar("There is no user record corresponding to this identifier",'error')
      }
      else{
        this.openSnackBar(err.message,'error')
      }
